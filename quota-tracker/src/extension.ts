...
interface CurrentQuotaState {
  gemini?: ParsedModelQuota;
  claude?: ParsedModelQuota;
  lastUpdated: Date;
  rawResponse?: QuotaSummaryResponse;
}
 ...
 */
async function fetchUserQuotaSummary(forceRefresh = false): Promise<QuotaSummaryResponse> {
  if (!cachedLsInfo) {
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode && res.statusCode >= 200 && res.statusCode < 300) {
          try {
            const parsed = JSON.parse(data) as QuotaSummaryResponse;
            resolve(parsed);
          } catch (e) {
...
function parseModelBucket(group: QuotaGroup): ParsedModelQuota | undefined {
  ...
  const fiveHourBucket = group.buckets.find(b =>
    b.window === '5h' ||
...
function parseQuotaSummary(data: QuotaSummaryResponse): { gemini?: ParsedModelQuota; claude?: ParsedModelQuota } {
  const groups = data.response?.groups ?? [];
  let gemini: ParsedModelQuota | undefined;
  let claude: ParsedModelQuota | undefined;
 ...
 */
function formatTimeRemaining(isoDateStr?: string): string {
  if (!isoDateStr) {
    return '--';
  }
 ...
 */
function formatLocalResetDate(isoDateStr?: string): string {
...
class QuotaTrackerExtension {
  ...
  private uiTickTimer: NodeJS.Timeout | null = null;
  ...
  public async refresh(forceServerRefresh = false, userInitiated = false): Promise<void> {
  ...
  public dispose() {
    if (this.fetchIntervalTimer) clearInterval(this.fetchIntervalTimer);
    if (this.uiTickTimer) clearInterval(this.uiTickTimer);
